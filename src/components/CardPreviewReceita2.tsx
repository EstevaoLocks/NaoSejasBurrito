// Imagens
import imgTacosMexicanos from './../assets/img/tacosMexicanos.png'
import imgChilliMexicano from './../assets/img/Chilli-mexicano.jpg'
import imgPozole from './../assets/img/pozole.jpg'
import imgGuacamole from './../assets/img/guacamole-original.jpg'
import imgBanderillasMexicanas from './../assets/img/banderillas-mexicnas.jpg'

// classe com os dados e funcoes de retorno
import { Receitas } from '../models/Receitas'

type CardPreviewReceitaProps = {
    receita: "taco" | "chiliMexicano" | "pozole" | "guacamole" | "banderillasMexicanas";
    versao: "v1" | "v2";
}

const Receita = new Receitas;
function expandirCard(event: React.MouseEvent<HTMLAnchorElement>) {
    event.preventDefault();

    const card = event.currentTarget.closest("article");
    if (card) {
        if (!card.classList.contains("active")) {
            // Antes de expandir, pega a altura atual e armazena como atributo data
            const currentHeight = card.clientHeight;
            card.dataset.initialHeight = currentHeight + "px";
            
            card.style.height = currentHeight + "px";
            card.classList.add("active");
            
            // Força um reflow e depois define a altura total
            card.offsetHeight; // Isso força o navegador a recalcular
            card.style.height = card.scrollHeight + "px";

            const lastElement = card.lastElementChild;
            if (lastElement) {
                const linkElement = lastElement.querySelector(".verMaisPratosTipicos");
                if (linkElement && linkElement.children[0]) {
                    linkElement.children[0].innerHTML = "Ver menos";
                }
            }
    
        } else {
            // Para recolher, usa a altura inicial armazenada
            const initialHeight = card.dataset.initialHeight || "200px"; // fallback

            card.style.height = card.clientHeight + "px";
            card.classList.remove("active");

            // Força um reflow e depois define a altura inicial
            card.offsetHeight;
            card.style.height = initialHeight; // Altura inicial

            const lastElement = card.lastElementChild;
            if (lastElement) {
                const linkElement = lastElement.querySelector(".verMaisPratosTipicos");
                if (linkElement && linkElement.children[0]) {
                    linkElement.children[0].innerHTML = "Ver mais";
                }
            }
        }

    }
}// end function expandirCard()

export function CardPreviewReceita2( { receita, versao }: CardPreviewReceitaProps ) {
    
    // Iniciando variavel 'imgReceita'
    let imgReceita;

    switch (receita) {
        case "taco":
            imgReceita = imgTacosMexicanos;
            break;

        case "chiliMexicano":
            imgReceita = imgChilliMexicano;
            break;
            
        case "pozole":
            imgReceita = imgPozole;
            break;
            
        case "guacamole":
            imgReceita = imgGuacamole;
            break;
            
        case "banderillasMexicanas":
            imgReceita = imgBanderillasMexicanas;
            break;
            
        default:
            imgReceita = imgTacosMexicanos;
            break;
    }

    return (
        <div className={`cardPratosTipicos ${versao}`}>
            <article>
                <header>
                    <h3>{Receita.getReceita(receita, "nome")}</h3>
                    <h4>{Receita.getReceita(receita, "categoria")}</h4>
                </header>
                <div className="descricao">{Receita.getReceita(receita, "descricao")}</div>
                <div className="blurPratosTipicos"></div>
                <footer>
                    <a 
                        href=""
                        className="verMaisPratosTipicos link-container"
                        onClick={expandirCard}
                    >
                        <p>Ver mais</p>
                    </a>

                    <div className="link-container">
                        <a href="receitas/lanches/Taco.html">
                            {/* Ícone de tijela */}
                            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12 12.2339H2.75C2.75 16.8419 8.5485 20.8039 8.5485 20.8039H12" stroke="black" stroke-width="0.5" stroke-linecap="round" stroke-linejoin="round"/>
                                <path d="M5.44202 13.2867C5.44202 16.6827 9.25602 19.1887 9.25602 19.1887M12 12.2337H21.25C21.25 16.8417 15.4515 20.8037 15.4515 20.8037H12M7.04702 12.2337C6.37402 9.71422 10.874 7.98222 14.912 7.44722L15.307 6.72021L16.899 8.65522L16.0255 8.79721C15.598 10.2022 13.458 12.2337 13.458 12.2337" stroke="black" stroke-width="0.5" stroke-linecap="round" stroke-linejoin="round"/>
                                <path d="M7.755 12.2336C7.899 10.9366 9.403 9.47662 15.141 7.72412" stroke="black" stroke-width="0.5" stroke-linecap="round" stroke-linejoin="round"/>
                                <path d="M15.289 7.90477C13.119 8.99177 10.589 10.2408 8.81604 12.2338M15.7125 8.41827C13.4575 11.4813 12 12.2338 12 12.2338M10.7525 12.2338C12.1125 11.4388 15.4975 8.15677 15.4975 8.15677M16.5385 8.22627L20.614 4.92377C20.681 4.86961 20.7367 4.80275 20.7778 4.72704C20.819 4.65132 20.8447 4.56824 20.8537 4.48255C20.8627 4.39685 20.8546 4.31023 20.8301 4.22765C20.8055 4.14506 20.7649 4.06814 20.7105 4.00127C20.6564 3.93434 20.5895 3.87874 20.5139 3.83766C20.4382 3.79658 20.3552 3.77082 20.2695 3.76186C20.1839 3.7529 20.0974 3.76091 20.0148 3.78543C19.9323 3.80995 19.8554 3.85051 19.7885 3.90477L15.7125 7.20727" stroke="black" stroke-width="0.5" stroke-linecap="round" stroke-linejoin="round"/>
                                <path d="M14.9125 7.44727L16.0255 8.79727" stroke="black" stroke-width="0.5" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
            
                            <p>Veja a receita</p>
                        </a>
                    </div>
                </footer>
            </article>
            <figure>
                <img src={imgReceita} alt=""/>
            </figure>
        </div>
    )
}